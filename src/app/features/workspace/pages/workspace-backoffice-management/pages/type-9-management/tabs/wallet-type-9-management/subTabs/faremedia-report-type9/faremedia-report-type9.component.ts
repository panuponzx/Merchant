...
import { CustomColumnModel, ILogRowModel, RowActionEventModel, ILogModel, IResponseLogModel } from 'src/app/core/interfaces';
...
export class FaremediaReportType9Component {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['refreshTrigger'] && !changes['refreshTrigger'].firstChange && this.faremediaValue) {
      this.onSearch();
    }
  }
  ngOnInit() {
  ...
  _onSearch() {
    ...
    return this.restApiService.postBackOffice("customer-type-9/get-log-faremedia", payload) as Observable<IResponseLogModel>;
  }
}
